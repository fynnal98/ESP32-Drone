...
void MPU6050::begin(int sda, int scl)
{
    ...
    // interner DLPF im Sensor (optional, hilft schon viel)
    m_mpu.enableGyrDLPF();
...
void MPU6050::update()
{
    ...
    // Gyro: raw -> °/s (bei 500 dps Range → 65.5 LSB/°/s)
    ...
    {
        Vec3f gIn{ gyroX, gyroY, gyroZ };
        Vec3f gOut = m_gyroLpf.update(gIn);
        gyroX = gOut.x;
        gyroY = gOut.y;
        gyroZ = gOut.z;
    }
    ...
    if (!m_initialized)
    {
        ...
        // Filterzustand sauber initialisieren
    ...
    m_yaw = 0.0f; // yaw bräuchte Magnetometer
}
